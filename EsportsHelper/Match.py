...
def remove_video_player_placeholder(driver):
	"""
	This function injects a JavaScript snippet into the page to continuously monitor
	for the creation of an element with ID 'video-player-placeholder' and remove it
	...
	driver.execute_script("""
		...
		function removeElementIfExists() {
			var element = document.getElementById('video-player-placeholder');
			if (element) {
				element.remove();
			}
		}
		...
		var observer = new MutationObserver(function(mutationsList) {
			for (var mutation of mutationsList) {
				if (mutation.type === 'childList') {
					// Check for the existence of the element after each change in the DOM
		...
		// Initial check in case the element is already present at script execution
		removeElementIfExists();
	""")
	...
	def watchMatches(self):
		...
		try:
			...
			while config.maxRunHours < 0 or time.time() < endTimePoint:
				...
				remove_video_player_placeholder(self.driver)
